...
**Tools:** `get_campaign_analytics`, `get_campaign_group_analytics`, `get_account_analytics`, `get_creative_analytics` (funnel: impressions, clicks, CTR, cost, leads, conversions); `get_demographic_analytics` (pivots: `MEMBER_JOB_TITLE`, `MEMBER_JOB_FUNCTION`, `MEMBER_SENIORITY`, `MEMBER_INDUSTRY`, `MEMBER_COMPANY_SIZE`, `MEMBER_COUNTRY_V2`, `MEMBER_REGION_V2`); `get_company_performance` (which companies saw/clicked — **resolves org IDs to readable company NAMES** by default via a batched `/organizations` lookup, `resolve_names=False` to skip); `get_lead_gen_performance` (form opens/completions). Note: demographic/company pivots come back as numeric LinkedIn org/title/industry IDs; `get_company_performance` is the one that auto-resolves names — for other pivots map IDs yourself or use the facet-ID reference above. Use `time_granularity="ALL"` for demographics, `"DAILY"` to see pacing.
