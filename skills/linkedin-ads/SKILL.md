...
**Tools:** `get_campaign_analytics`, `get_campaign_group_analytics`, `get_account_analytics`, `get_creative_analytics` (funnel: impressions, clicks, CTR, cost, leads, conversions); `get_demographic_analytics` (pivots: `MEMBER_JOB_TITLE`, `MEMBER_JOB_FUNCTION`, `MEMBER_SENIORITY`, `MEMBER_INDUSTRY`, `MEMBER_COMPANY_SIZE`, `MEMBER_COUNTRY_V2`, `MEMBER_REGION_V2` — URN-based pivots now auto-**resolve to names** via `adTargetingEntities?q=urns`, `resolve_names=False` to skip; `MEMBER_COMPANY_SIZE` is already a readable enum); `get_company_performance` (which companies saw/clicked — **resolves org IDs to readable company NAMES** by default via a batched `/organizations` lookup, `resolve_names=False` to skip); `get_lead_gen_performance` (form opens/completions). Name resolution (June 2026): `get_company_performance` resolves org IDs→company names via `/organizations`; `get_demographic_analytics` resolves title/industry/seniority/function/geo URNs→names via `adTargetingEntities?q=urns`. Both default on (`resolve_names=False` to skip the extra lookups). `MEMBER_COMPANY_SIZE` is already a readable enum (`SIZE_201_TO_500`). Use `time_granularity="ALL"` for demographics, `"DAILY"` to see pacing.
